CLASS zcl_ce_rap_agency_osap DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_agency_range TYPE RANGE OF zrap_osapz_travel_agency_es5-agencyid.
    TYPES t_business_data TYPE TABLE OF zrap_osapz_travel_agency_es5.
...
CLASS ZCL_CE_RAP_AGENCY_OSAP IMPLEMENTATION.
  ...
  METHOD get_agencies.
    ...
    DATA: lo_http_client     TYPE REF TO if_web_http_client,
        ...
        DATA(lo_http_destination)  = cl_http_destination_provider=>create_by_cloud_destination( i_name = 'ES5' i_authn_mode = if_a4c_cp_service=>service_specific ).

        lo_http_client = cl_web_http_client_manager=>create_by_http_destination( i_destination = lo_http_destination ).

        service_consumption_name = to_upper( 'ZSC_RAP_AGENCY_osap' ).
